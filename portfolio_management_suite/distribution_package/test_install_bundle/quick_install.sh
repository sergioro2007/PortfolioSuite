...
elif [ -f "../portfolio_management_suite-2.0.0.tar.gz" ]; then
    python3 -m pip install --no-cache-dir --index-url https://pypi.org/simple ../portfolio_management_suite-2.0.0.tar.gz
else
    echo "❌ Neither .whl nor .tar.gz found. Please ensure the install package is in the same directory as this script."
    exit 1
fi
