...
class TestMarketHealthAnalysis(unittest.TestCase):
    ...
    # Helper to mimic the old get_defensive_criteria logic
    def get_defensive_criteria(self, market_health):
        regime = market_health.get('market_regime', 'AGGRESSIVE')
        if regime == 'HIGHLY_DEFENSIVE':
            return {'max_results': 5, 'min_rs_score': 40, 'min_weekly_target': 0.5}
        elif regime == 'DEFENSIVE':
            return {'max_results': 8, 'min_rs_score': 35, 'min_weekly_target': 0.8}
        else:
            return {'max_results': 25, 'min_rs_score': 20, 'min_weekly_target': 1.2}
    ...
    def test_get_defensive_criteria_aggressive(self):
        ...
        criteria = self.get_defensive_criteria(market_health)
