...
from portfolio_suite.tactical_tracker.core import (
    PortfolioTracker,
    discover_momentum_tickers,
    screen_discovered_tickers,
    get_sp500_leaders,
    get_etf_leaders,
    get_momentum_stocks
)
...
class TestTickerDiscovery(unittest.TestCase):
    ...
    def test_discover_momentum_tickers(self):
        ...
        tickers = discover_momentum_tickers()
    ...
    def test_screen_discovered_tickers(self, mock_analyze):
        ...
        results = screen_discovered_tickers(
            self.tracker,
            test_tickers,
            min_rs_score=70,
    ...
    def test_filter_by_rules_basic(self):
        """Test basic filtering by momentum rules (modernized)"""
        # Create mock results for different ticker performance
        results = [
            {'ticker': 'STRONG', 'rs_score': 80, 'avg_weekly_return': 3.0, 'market_cap': 1e11, 'meets_criteria': True, 'weeks_above_target': 3, 'daily_change': 1.5, 'weekly_returns': [0.03, 0.025, 0.035, 0.028]},
            {'ticker': 'WEAK', 'rs_score': 10, 'avg_weekly_return': 0.5, 'market_cap': 1e9, 'meets_criteria': False, 'weeks_above_target': 0, 'daily_change': -0.5, 'weekly_returns': [0.0, 0.0, 0.0, 0.0]}
        ]
        # Use get_top_picks to filter
        top_picks = self.tracker.get_top_picks(results, count=10)
        self.assertIsInstance(top_picks, list)
        # Should only return the strong ticker
        self.assertEqual(len(top_picks), 1)
        self.assertEqual(top_picks[0]['ticker'], 'STRONG')
