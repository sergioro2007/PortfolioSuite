...
class OptionsTracker:
    ...
    def get_technical_indicators(self, ticker: str, period: str = "3mo") -> Dict:
        ...
        try:
            ...
            if hist.empty:
                print(f"No historical data for '{ticker}' (sanitized: '{ticker_clean}')")
                return {}
    ...
    def generate_dynamic_watchlist(self) -> Dict:
        ...
        def add_ticker(ticker, verbose=True):
            try:
                ticker_data = self._calculate_ticker_parameters(ticker)
                if ticker_data:
                    watchlist[ticker] = ticker_data
                    if verbose:
                        print("Added {} to watchlist: Price=${:.2f}, Range=${:.2f}-${:.2f}, Target=${:.2f}, Bias={:.2f}".format(
                            ticker,
                            ticker_data['current_price'],
                            ticker_data['range_68'][0],
                            ticker_data['range_68'][1],
                            ticker_data['target_zone'],
                            ticker_data['bias_prob']
                        ))
            except Exception as e:
                if verbose:
                    print("Error adding {} to watchlist: {}".format(ticker, e))

        for ticker in popular_tickers:
            add_ticker(ticker)


        # To further expand the watchlist with more high-volume tickers, use:
        # self.expand_watchlist_with_high_options_volume(min_additional_tickers=5)
